...
def filter_trilingual_dataset_indices_by_size(
    src_sizes, 
    tgt1_sizes, 
    tgt2_sizes, 
    indices, 
    ...
    if type(max_sizes) in (int, float):
        ...
        max_tgt1_size = max_sizes
        max_tgt2_size = max_sizes
    elif len(max_sizes) == 3:
        max_src_size, max_tgt1_size, max_tgt2_size = max_sizes
    else:
        max_src_size, max_tgt_size = max_sizes
        max_tgt1_size = max_tgt_size
        max_tgt2_size = max_tgt_size
    if tgt1_sizes is None and tgt2_sizes is None:
        ignored = indices[src_sizes[indices] > max_src_size]
    else:
        ignored = indices[
            (src_sizes[indices] > max_src_size) 
            | (tgt1_sizes[indices] > max_tgt1_size)
            | (tgt2_sizes[indices] > max_tgt2_size)
        ]
    if len(ignored) > 0:
        if tgt1_sizes is None and tgt2_sizes is None:
            indices = indices[src_sizes[indices] <= max_src_size]
