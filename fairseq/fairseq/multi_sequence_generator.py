...
class MultiSequenceGenerator(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        if isinstance(models, EnsembleMultiDecoderModel):
            self.model = models
        else:
            self.model = EnsembleMultiDecoderModel(models)
    ...
    def _generate(
        ...
        incremental_states1 = torch.jit.annotate(
        ...
        incremental_states2 = torch.jit.annotate(
            List[Dict[str, Dict[str, Optional[Tensor]]]],
            [
                torch.jit.annotate(Dict[str, Dict[str, Optional[Tensor]]], {})
                for i in range(self.model.models_size)
        ...
        new_order1 = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order1 = new_order1.to(src_tokens.device).long()
        ...
        encoder_outs1 = self.model.reorder_encoder_out(encoder_outs, new_order1)
        ...
        # bbsz2_offsets = (torch.arange(0, bsz) * beam_size).unsqueeze(1).type_as(tokens2).to(src_tokens.device)
        cand2_offsets = torch.arange(0, cand_size).type_as(tokens2).to(src_tokens.device)

        reorder_state1: Optional[Tensor] = None
        reorder_state2: Optional[Tensor] = None
        ...
        def decode_step(
            ...
            }, encoder_outs, tokens, scores, attn
        ...
        def update_active_cands(
            ...
            return cands_to_ignore, tokens, scores, attn, reorder_state

        for step in range(max_len + 1):  # one extra step for EOS marker
            step_out1, encoder_outs1, tokens1, scores1, attn1 = decode_step(
                step,
                encoder_outs1,
                0,
                tokens1,
                ...
                bbsz_offsets,
                original_batch_idxs,
                reorder_state=reorder_state1,
                prefix_tokens=None if prefix_tokens is None else prefix_tokens[0],
            )
            step_out2, encoder_outs2, tokens2, scores2, attn2 = decode_step(
                step,
                encoder_outs2,
                1,
                tokens2,
            ...
            cands_to_ignore1, tokens1, scores1, attn1, reorder_state1 = update_active_cands(
                step,
                cands_to_ignore1,
                step_out1["eos_mask"],
                cand1_offsets,
            ...
            cands_to_ignore2, tokens2, scores2, attn2, reorder_state2 = update_active_cands(
                step,
                cands_to_ignore2,
                step_out2["eos_mask"],
                cand2_offsets,
...
class EnsembleMultiDecoderModel(nn.Module):
    ...
    def __init__(self, models):
        ...
        if all(
            hasattr(m, "decoders") and 
            isinstance(m.decoders[0], FairseqIncrementalDecoder) and
            isinstance(m.decoders[1], FairseqIncrementalDecoder)
            for m in models
        ):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                decoder_out = model.decoders[lang].forward(
                    tokens,
                    encoder_out=encoder_out,
                    incremental_state=incremental_states[i],
                )
            else:
                decoder_out = model.decoders[lang].forward(tokens, encoder_out=encoder_out)
            ...
            probs = model.get_normalized_probs(
                decoder_out_tuple, log_probs=True, lang=lang, sample=None
            )
    ...
    def reorder_incremental_state(
        ...
        if not self.has_incremental_states():
            return
        for i, model in enumerate(self.models):
            model.decoders[lang].reorder_incremental_state_scripting(
